...
create definer = administrator@localhost procedure delete_record_from_client_order_room(IN orderID int, IN roomID int)
BEGIN
    declare result int;
    SELECT COUNT(1)
    into result
...
create definer = administrator@localhost procedure get_free_rooms(IN dateFrom date, IN dateTo date)
BEGIN
...
create definer = administrator@localhost procedure insert_new_order(IN client_id_in bigint, IN room_id_in int, IN check_in_date_in date, IN check_out_date_in date, IN order_status_in char(20), OUT id int)
BEGIN
